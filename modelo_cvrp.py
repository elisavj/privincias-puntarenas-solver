...
Restricción de tiempo incluida de DOS formas:
  Opción A — restricción explícita en el MIP:
             ...
             ⚠️  DESACTIVADA por defecto: aumenta km totales sin beneficio
...
# ── Resolver CVRP ────────────────────────────────────────────
def resolver_cvrp(time_limit: int = 180, respetar_jornada: bool = False) -> dict:
    """
    ...
    respetar_jornada : si True, agrega restricción A (tiempo por arco ≤ 480 min)
                       NOTA: dejarlo en False minimiza la distancia total (5 388 km).
                       Con True el solver fragmenta rutas y aumenta los km totales.
    ...
    #     ⚠️  Desactivada por defecto (aumenta km sin mejorar la solución operativa)
    if respetar_jornada:
        for (i, j) in ARCOS:
            if i != 0 and j != 0:
...
def _reconstruir_rutas(arcos: dict, N: list) -> list[dict]:
    ...
    # Paso 3: dividir en sub-trips si excede CAP
    rutas = []
...
if __name__ == "__main__":
    print("Resolviendo CVRP — objetivo: minimizar distancia total…")
    res = resolver_cvrp()
