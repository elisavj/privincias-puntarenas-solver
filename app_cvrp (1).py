...
respetar_jornada = st.sidebar.toggle("Restricción de jornada en MIP (Opción A)", value=True)
...
with tab3:
    if res is None:
        ...
        for idx, t in enumerate(trucks):
            ...
            header = (f"**Camión {idx+1}** &nbsp;·&nbsp; {t['tipo']} &nbsp;·&nbsp; "
                      f"{t['tiempo']:.0f} / {JORNADA} min ({ocup:.0f}%)")
            with st.expander(f"Camión {idx+1} — {t['tipo']} — {t['tiempo']:.0f} min"):
                # Barra de ocupación de jornada
                df_bar = pd.DataFrame({
        ...
        gantt = (
            ...
            .encode(
                x=alt.X("Utilizado:Q", title="Minutos utilizados",
                         scale=alt.Scale(domain=[0, JORNADA + 30])),
        ...
        st.caption(f"Línea blanca = límite de jornada ({JORNADA} min = 8 h) · Cada barra = tiempo utilizado por camión")
...
(5) [Opción A] Tiempo: si t_est(i→j) > {JORNADA} min → y(i,j) = 0
```
