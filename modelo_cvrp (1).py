...
def _make_trip(ruta: list[int]) -> dict:
    """Construye el dict de un trip a partir de su lista de nodos."""
    km      = sum(DIST_RAW[ruta[k]][ruta[k+1]] for k in range(len(ruta)-1))
    pallets = sum(DEMANDA[n] for n in ruta if n != 0)
...
def _split_demand_into_trips() -> list[dict]:
    """
    Genera trips directos CD→cliente→CD para cada cliente,
    dividiendo su demanda en cargas de exactamente CAP pallets
    (el último trip lleva el residuo).
    Respeta estrictamente la capacidad de 24 pallets por trip.
    """
    trips = []
    for nodo in range(1, 14):
        ...
        while dem > 0:
            ...
            trips.append({
                "nodos":    [0, nodo, 0],
                "nombres":  [CANTONES[0], CANTONES[nodo], CANTONES[0]],
                "km":       DIST_RAW[0][nodo] * 2,
                "pallets":  carga,
                "duracion": round((DIST_RAW[0][nodo] * 2 / VELOCIDAD * 60)
                                  + 1 * T_PARADA + carga * T_PALLET, 1),
                "dedicado": False,
            })
            dem -= carga
    # Marcar dedicados
    for t in trips:
        t["dedicado"] = t["duracion"] > JORNADA
    return trips
