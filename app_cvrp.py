...
st.sidebar.markdown("---")
st.sidebar.subheader("📦 Demanda por cantón")
for k in range(1, 14):
    st.sidebar.markdown(
        f"<div style='display:flex;justify-content:space-between;padding:3px 0;"
        f"border-bottom:1px solid #880e4f22;'>"
        f"<span style='color:#fce4ec;font-size:0.85rem'>{CANTONES[k]}</span>"
        f"<span style='color:#f48fb1;font-weight:700;font-size:0.85rem'>{DEMANDA[k]} pal</span>"
        f"</div>",
        unsafe_allow_html=True
    )
st.sidebar.markdown("---")
...
with tab1:
    if res is None:
        ...
        st.markdown("---")
        col_trips, col_dem = st.columns([2, 1])
        ...
        with col_trips:
            ...
            for idx, r in enumerate(res["rutas"]):
                rows_trips.append({
                    "Trip":     idx + 1,
                    "Tipo":     "🔴 Dedicado" if r["dedicado"] else "🟢 Normal",
                    "Ruta":     " → ".join(r["nombres"]),
                    "Pallets":  r["pallets"],
                    "km":       r["km"],
                    "Duración (min)": round(r["duracion"]),
                })
            st.dataframe(pd.DataFrame(rows_trips), use_container_width=True, hide_index=True)

        with col_dem:
            st.subheader("📦 Demanda optimizada")
            df_dem2 = pd.DataFrame([
                {
                    "Cantón":           CANTONES[k],
                    "Imperial (50%)":   round(DEMANDA[k] * 0.50),
                    "Pilsen (25%)":     round(DEMANDA[k] * 0.25),
                    "Tropical (25%)":   round(DEMANDA[k] * 0.25),
                    "Total":            DEMANDA[k],
                }
                for k in range(1, 14)
            ])
            st.dataframe(
                df_dem2.style.bar(subset=["Total"], color="#880e4f", vmin=0),
